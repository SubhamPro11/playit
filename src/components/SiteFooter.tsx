...
export const SiteFooter: React.FC<SiteFooterProps> = ({ totalVideos }) => {
  ...
  return (
    <footer className="border-t border-[#26262a] bg-[#08080a] text-zinc-400 py-12 px-4 sm:px-6 lg:px-8 mt-16 font-sans">
      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-12 gap-8 lg:gap-12">
        ...
        <div className="md:col-span-3">
          <h4 className="font-mono text-xs font-semibold text-zinc-300 uppercase tracking-wider mb-3">
            Collection index
          </h4>
          <ul className="space-y-2 text-xs font-mono text-zinc-400">
            <li className="flex items-center justify-between">
              <span>Total curated links:</span>
              <span className="text-red-400 font-bold">{totalVideos}</span>
            </li>
            <li className="flex items-center justify-between">
              <span>Genre categories:</span>
              <span className="text-zinc-200">{realCategoriesCount}</span>
            </li>
            <li className="flex items-center justify-between">
              <span>Playback model:</span>
              <span className="text-zinc-200">External direct links</span>
            </li>
            <li className="flex items-center justify-between">
              <span>Favorites storage:</span>
              <span className="text-zinc-200">Local browser only</span>
            </li>
          </ul>
        </div>
        ...
        <div className="md:col-span-4">
          ...
          <div className="flex flex-wrap gap-1.5">
            {CATEGORIES.filter((c) => c !== 'All').map((cat) => (
              <span
                key={cat}
                className="px-2.5 py-1 rounded-lg bg-[#141418] border border-[#27272a] text-[11px] font-mono text-zinc-400"
              >
                {cat}
              </span>
            ))}
          </div>
          <div className="mt-4 pt-3 border-t border-[#27272a] text-[11px] font-mono text-zinc-500">
      ...
      {/* Bottom Bar */}
      <div className="max-w-7xl mx-auto mt-10 pt-6 border-t border-[#26262a] flex flex-col sm:flex-row items-center justify-between gap-4 text-xs font-mono text-zinc-500">
        <div>

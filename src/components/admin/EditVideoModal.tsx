...
interface EditVideoModalProps {
  ...
  isOpen: boolean;
  onClose: () => void;
  onSave: (id: string, updates: Partial<Video>) => Promise<boolean>;
}
...
export const EditVideoModal: React.FC<EditVideoModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    const success = await onSave(video.id, {
      title: title.trim(),
      externalLink: externalLink.trim(),
      thumbnailUrl: thumbnailUrl.trim(),
      category,
      orderIndex: Number(orderIndex) || video.orderIndex,
    });
        ...
        <div className="px-5 py-4 border-b border-surface-700 flex items-center justify-between">
          <div>
            ...
            <p className="font-mono text-xs text-slate-400">
              Editing #{String(video.orderIndex).padStart(2, '0')} · {video.title}
            </p>
          </div>
          <button
            onClick={onClose}
